...
description: Sitemap and robots.txt for App Router + Cache Components: src/app/api/sitemap/route.ts and src/app/api/robots/route.ts with createSitemapRouteHandler and createRobotsRouteHandler. Rewrites in next.config.ts. Use when configuring sitemap, robots.txt, or SEO.
---

# Content SDK Sitemap and Robots (App Router + Cache Components)
...
- With Cache Components enabled (`cacheComponents: true` in `next.config.ts`), the explicit `export const dynamic = 'force-dynamic'` on these route files is **not** needed — Next.js infers dynamic/static behavior automatically.
